    ...
    const addDefer = defer => {
      ...
      if (defer.timeout) {
        timer = setTimeout(() => {
          defer.invoked = true;
          defer.failed = true;
          defer.error = new Error(`runDefer timed out after ${defer.timeout}ms`);
          return invokeFinally(defer.error);
        }, defer.timeout).unref();
      }
    };
    ...
    if (checkFunc.hasOwnProperty(WRAPPED_FN)) {
      ...
      if (wrap._timeout) {
        ...
        timeoutTimer = setTimeout(() => {
          failError = new Error(`runVerify: test timeout after ${wrap._timeout}ms`);
        ...
        return new Promise((resolve, reject) => {
          if (d.invoked) {
            ...
            reset = removeEvent => {
              clearTimeout(timer);
              d._waiting = false;
              event.removeListener(removeEvent, rejectCb);
            };
            ...
            if (waitTimeout > 0) {
              timer = setTimeout(
                () => rejectCb(new Error(`defer wait timeout after ${waitTimeout}ms`)),
                waitTimeout
              ).unref();
            }
          }
